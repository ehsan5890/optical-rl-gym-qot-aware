...
def run_with_callback(callback, env_args, num_eps, log_dir):
    ...
    if callback is phy_aware_bmff_rmsa:
        ...
        env = gym.make("PhyRMSA-v0", **env_args)
        env = Monitor(env, log_dir + 'BM-FA-Cut-modified-no-gr', info_keywords=('episode_service_blocking_rate','service_blocking_rate',
                                                                                                 'episode_bit_rate_blocking_rate', 'number_cuts_total', 'rss_total_metric',
                                                                                                  'total_path_length',
                                                                                                    'num_moves',
                                                                 'num_defrag_cycle','avrage_gsnr', 'average_mod_level', 'average_path_index', 'path_index','physical_paths', 'num_moves_groom'))
        ...
        env = gym.make("PhyRMSA-v0", **env_args)
        env = Monitor(env, log_dir + 'FA-FF-RSS', info_keywords=('episode_service_blocking_rate','service_blocking_rate',
                                                                                                 'episode_bit_rate_blocking_rate', 'number_cuts_total', 'rss_total_metric',
                                                                                                  'total_path_length',
                                                                                                    'num_moves',
                                                                 'num_defrag_cycle','avrage_gsnr', 'average_mod_level', 'average_path_index', 'path_index','physical_paths', 'num_moves_groom'))
        ...
        env = gym.make("PhyRMSA-v0", **env_args)
        env = Monitor(env, log_dir + 'BM-FA-RSS-no-gr', info_keywords=('episode_service_blocking_rate','service_blocking_rate',
                                                                                                 'episode_bit_rate_blocking_rate', 'number_cuts_total', 'rss_total_metric',
                                                                                                 'total_path_length',
