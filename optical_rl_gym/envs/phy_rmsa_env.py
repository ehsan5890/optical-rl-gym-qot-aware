...
## here 1: BVTs in C band, 0: BVTs in L band, 2: BVTs in S band
    ...
    def calculate_r_spatial(self, channel_number, link_indexes):
        r_spatial = 0
        r_spatial_after = 0
...
# first to check if we have enough resource in virtual layer
    ...
    if env.grooming_layer[env.current_service.source_id][env.current_service.destination_id] >= env.current_service.bit_rate:
        env.grooming_layer[env.current_service.source_id][env.current_service.destination_id] -= env.current_service.bit_rate
        ...
        selected_channels = []
        table_id = np.where(((env.connections_detail[:, 0] == int(env.current_service.source)) & (env.connections_detail[:, 1] == int(env.current_service.destination))) | (
                    (env.connections_detail[:, 0] == int(env.current_service.destination)) & (env.connections_detail[:, 1] == int(env.current_service.source))))[0][0]
    ...
    if env.grooming_layer[env.current_service.source_id][env.current_service.destination_id] >= env.current_service.bit_rate:
        ...
        while True:
            ...
            for i, row in enumerate(sorted_free_channels):
                # Check if the modulation level of the first tuple in the row is greater than or equal to the current maximum
                if row != [] and (row[0][0] > max_modulation_level or (
                        row[0][0] == max_modulation_level and row[0][1] >  max_frag_metric)): # it was i < max_modulation_row_index
                    max_modulation_level = row[0][0]
